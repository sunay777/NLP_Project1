...
\section{Methods}
\looseness=-1
\textbf{Task.} We use \citeauthor{singh2024needs}'s study$\to$query symbol task: $K{=}4$ symbols (drawn from a pool of 16) are each given a random label (from 6) for one sequence only; a study block presents the $K$ pairs in one order, a query block re-presents the same symbols in an \emph{independent} order with labels withheld. Because the match offset varies with the query permutation, only content-based matching -- not a fixed positional shortcut -- solves the task. In \emph{extended} mode the model additionally free-generates one more symbol from the $K$ shown, and its label, which we call $sq2$; $sq2$ is drawn uniformly at random and is therefore irreducible to predict, giving a provable loss floor of $\ln K/(K{+}2) \approx 0.231$ for $K{=}4$ (confirmed empirically to within $0.002$ across $K \in \{2,\ldots,5\}$, independent of vocabulary size). Reaching this clean circuit required four choices verified by ablation during development: rotary position embeddings (previous-token attention plateaus at $\sim$0.3 with absolute positions), tied input/output embeddings, a cosine-decayed learning rate (a constant rate destabilises an already-formed circuit), and the independent study/query permutation itself (a fixed offset lets the model solve the task positionally, with zero content-matching).

\textbf{Model.} A 2-layer, 4-head, attention-only transformer ($d{=}64$, $\sim$35.6k parameters). Hyper-parameters were chosen by grid search (learning rate, warmup, width) against validation loss with 2 seeds, then confirmed once on a disjoint seed: the extended-mode grid contained near-indistinguishable validation losses because most configurations sit at the loss floor above, and the top-ranked configuration by validation loss in fact failed to form the circuit on the held-out seed (test accuracy 0.63) -- validation loss cannot rank configurations whose main failure mode is a stochastic non-transition, which motivates the convergence gate below.

\textbf{Collapse pipeline.} Generation 0 trains on fresh real data. Generation $g{\geq}1$ trains a \emph{fresh} model on a 20k-sequence pool built from a controlled \texttt{real\_fraction} of fresh real sequences and the remainder sampled from generation $g{-}1$ (every answer slot model-generated); we hold out 10\% of each pool for validation and evaluate on fresh real data as test. Because extended-mode transition timing is seed-dependent (900--2900 steps across seeds at a fixed budget, and one seed never transitioned within 6000 steps), each generation must clear a validation-accuracy gate or is retrained from a new initialisation (up to 3 retries); this prevents an undertrained generation from being misread as collapse. We sweep \texttt{real\_fraction} $\in \{0, 0.1, 0.25, 0.5, 1\}$ for 6 generations and 3 seeds, in both base and extended mode.
...
\begin{figure}[h!]
  ...
  \end{minipage}
  \caption{\looseness=-1 Base-mode circuit, generation 0. (a) All four layer-0 heads act as previous-token heads, attending a label back to its own preceding symbol and a symbol back to the preceding label (0.86--0.97). (b) Three of four layer-1 heads attend, from the query position, specifically to the matching label from the study block (0.88--0.95) rather than to the matching symbol or any fixed offset, ruling out a positional or direct-match shortcut. (c) Zero-ablating any single head leaves accuracy $\geq$0.96, but ablating all four layer-0 heads together drops it to 0.31 and all four layer-1 heads to 0.05 -- the circuit is causally load-bearing and redundant within each layer.}
  \label{fig:circuit}
\end{figure}
...
\caption{\looseness=-1 Extended mode, final generation (mean $\pm$ sd, 3 seeds). Conditional diversity and off-context mass move together and monotonically with \texttt{real\_fraction}; induction strength and query accuracy do not.}
...
Figure~\ref{fig:dose} shows this as a dose-response: conditional diversity and off-context mass move smoothly with \texttt{real\_fraction} (no sharp critical fraction), while induction strength for both extended and base mode sits in a narrow, flat band across the whole sweep. Re-probing all 30 saved final-generation models causally confirms this: every one retains a canonical or near-canonical circuit, with whole-layer-1 ablation costing 0.74--1.00 accuracy (extended) and 0.61--0.93 (base). Two further checks matter for these numbers. First, a finite-data confound: even the \texttt{real\_fraction}$=$1 control drops conditional diversity from 0.994 at generation 0 (unlimited fresh data) to $\sim$0.90 once generation $\geq$1 trains on a finite 20k pool ($\sim$77 epochs); the recursive effect we attribute to collapse is the residual gap to this control, not the raw drop from generation 0. Second, circuit multiplicity: 10/90 extended and 6/90 base generations converge on one of two non-canonical but fully accurate algorithms -- a ``shifted'' copy (matching one position later than the canonical head, using a $t{-}2$ head's information) or an ``elimination'' strategy (attending \emph{away} from the match and suppressing non-matching labels) -- and attention-pattern reading alone falsely calls the circuit collapsed in 9 and 6 of these respectively, even though causal ablation shows both variants are load-bearing.
...
\begin{figure}[h!]
  ...
  \end{minipage}
  \caption{\looseness=-1 (a) At the final generation, extended-mode conditional diversity (blue) sits well below the generation-0 ceiling and rises only gradually with \texttt{real\_fraction}, while extended induction strength (orange) and the base-mode induction control (green) stay flat near 0.94--0.97 throughout. (b) Off-context mass -- probability the model places on symbols it was never shown -- falls smoothly from $\sim$0.05 at \texttt{real\_fraction}$=$0 to the generation-0 floor of 0.012 at \texttt{real\_fraction}$=$1, the clearest single symptom of collapse in this task.}
  \label{fig:dose}
\end{figure}

\section{Discussion}
\looseness=-1
Recursive training on this task produces a \emph{dissociation}, which is our answer to the question posed in the introduction: the ICL circuit is not what collapses. Query labels are always fixed by the context, so every self-generated sequence re-validates the same copy computation; $sq2$ is under-determined by the context, so a model's own sampling noise becomes part of its successor's training signal, and it is there, not in the circuit, that collapse concentrates -- chiefly as \emph{off-context hallucination}, probability mass on symbols never shown, rather than as a sharper-but-still-in-context preference. The dose-response is graded rather than critical: roughly half real data already matches the all-real control on diversity, while off-context mass keeps improving all the way to \texttt{real\_fraction}$=$1, so there is no single fraction below which the task ``breaks''. Two methodological points generalise beyond this task. Marginal diversity is the natural, cheap diagnostic for collapse, but it is blind here because the generative slot is itself a uniform draw over context-dependent options; auditing an ICL-capable model for collapse needs a \emph{conditional} diagnostic, not a marginal one. Separately, an attention-pattern probe is algorithm-specific and can read a healthy, differently-implemented circuit as collapsed; causal ablation is necessary before any claim that a circuit has degraded. Limitations: a 35.6k-parameter model and a 16-symbol vocabulary, 6 generations, 3 seeds, sampling temperature 1 only, a fresh model per generation, zero- rather than mean-ablation, and generation-1 seeds shared across fractions (useful for paired comparison, but meaning a single unlucky initialisation, e.g. seed 2's elimination-circuit run, appears at every fraction). We have not identified what the sharpening of $sq2$ is keyed on beyond context membership; study- and query-slot entropies stay near 1 throughout, and we leave this, along with a deliberately seeded symbol bias at generation 0 (a data-poisoning framing) to measure amplification, for future work.
